...
const WRITING_DELIVERABLE_PATTERNS = [
  /\bemail\b/i,
  /\bmessage\b/i,
  /\bcover letter\b/i,
  /\bcaption\b/i,
...
const PROMPT_DELIVERABLE_PATTERNS = [
  /\bprompt\b/i,
  /\bresearch prompt\b/i,
  /\bcode prompt\b/i,
  /\brewrite prompt\b/i,
  /\bgenerate(?:d)? prompt\b/i,
  /\bsend-ready prompt\b/i
]

const ADVICE_DELIVERABLE_PATTERNS = [
  /\bproduct\b/i,
  /\bdrink\b/i,
  /\bmix\b/i,
  /\btea\b/i,
...
const STRUCTURED_PROMPT_SECTION_PATTERNS = [
  /task\s*\/\s*goal:/i,
  /key requirements:/i,
  /constraints:/i,
  /output format:/i,
...
export function classifyReviewTaskType(attempt: Pick<Attempt, "raw_prompt" | "optimized_prompt" | "intent">): ReviewTaskType {
  ...
  const unresolvedRuntimeIssue =
    matchesAny(searchText, DEBUG_PATTERNS) &&
    /\bstill\b|\bnot\b|\bmissing\b|\bdoesn'?t\b|\bcan'?t\b|\bfailing\b/i.test(searchText)
