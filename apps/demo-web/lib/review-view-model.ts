...
import type { ReviewPopupMode } from "./review-types"
...
function markerForChecklist(
  ...
  if (taskType === "debug" && mode === "quick") {
    if (status === "met" || status === "not_sure") return "not_verified"
    return "missing"
  }
...
function checklistLabel(item: AfterAnalysisResult["acceptance_checklist"][number]) {
...
function buildDecision(result: AfterAnalysisResult, mode: ReviewPopupMode, taskType: ReviewTaskType, qualityTask: boolean) {
  ...
  if (debugContinuation) {
    return mode === "deep" ? "This fix hasn’t been proven yet" : "Confirm one runtime checkpoint before trusting this fix"
  }
  ...
  if (mode === "deep") {
    ...
    if (result.status === "PARTIAL") {
      return confirmedCount === 0 ? "Key checklist items are still unproven" : "Some checklist items look right, but aren’t proven yet"
    }
  }
  if (result.status === "SUCCESS") return mode === "deep" ? "Looks correct — based on visible proof" : "Looks ready to continue"
...
function buildRecommendation(result: AfterAnalysisResult, mode: ReviewPopupMode, taskType: ReviewTaskType, qualityTask: boolean) {
  const debugContinuation = taskType === "debug" && result.prompt_strategy === "narrow_scope"
  if (debugContinuation) {
    return mode === "deep" ? "Check what actually happens in runtime before changing anything else." : "Check the most likely runtime gap before changing more code."
  }
  if (qualityTask) {
    if (result.status === "SUCCESS") return mode === "deep" ? "You can use this as-is." : "Continue, no changes needed."
    return mode === "deep" ? "Fix the unclear or missing parts before using this." : "Ask for the missing step or clarification only."
  }
  if (result.status === "SUCCESS") {
    return mode === "deep" ? "Proceed, but only trust what’s clearly supported." : "Keep moving, but stay alert for anything still lightly supported."
  }
  if (mode === "deep") {
...
function buildMissingItems(result: AfterAnalysisResult, mode: ReviewPopupMode) {
...
function buildProofChecked(result: AfterAnalysisResult, mode: ReviewPopupMode, taskType: ReviewTaskType, qualityTask: boolean) {
  if (taskType === "debug") {
    ...
    return [mode === "deep" ? "Code changes were described, but runtime verification is still thin." : "Checked: the assistant described code changes or the intended fix."]
  }
  if (qualityTask) {
    const addressed = result.stage_2.addressed_criteria.slice(0, 3)
    if (addressed.length) return addressed
    return [mode === "deep" ? "The answer was checked for clarity, completeness, and major omissions." : "The answer was checked for directness and basic usability."]
  }
  ...
  return [mode === "deep" ? "The answer explains an approach, but no concrete result proof was shown." : "The answer describes an approach."]
}
...
function buildProofMissing(result: AfterAnalysisResult, mode: ReviewPopupMode, taskType: ReviewTaskType, qualityTask: boolean) {
  if (taskType === "debug") {
    ...
    return [mode === "deep" ? "Live runtime confirmation is still missing." : "Not checked: the runtime result is still not confirmed."]
  }
  if (qualityTask) {
    const missing = result.stage_2.missing_criteria.slice(0, 3)
    if (missing.length) return missing
    return [mode === "deep" ? "No major omissions or unsafe steps stood out in this review." : "No obvious missing step stood out in this quick read."]
  }
  const missing = result.stage_2.missing_criteria.slice(0, 3)
  if (missing.length) return missing.map((item) => (mode === "deep" ? `No concrete proof was shown for: ${item}.` : item))
  return [mode === "deep" ? "The answer explains the fix, but not the outcome." : "Some proof is still lightweight in this quick read."]
}

function buildQuickToDeepDelta(quick: AfterAnalysisResult | null, deep: AfterAnalysisResult, qualityTask: boolean) {
  if (!quick) return qualityTask ? "Deep review used the same checklist with a stricter completeness and clarity check." : "Deep review used the same checklist with stronger proof expectations."
  ...
  return qualityTask ? "Deep review kept the same checklist but judged it with a stricter completeness standard." : "Deep review kept the same checklist but judged it with stronger proof standards."
}
...
function buildPromptCardText(result: AfterAnalysisResult) {
  ...
  const focusItems = [
    ...
    ...result.acceptance_checklist.filter((item) => item.status !== "met").map((item) => item.label)
  ]
    .map((item) => item.trim())
    .filter(Boolean)
  ...
  const alreadyMentionsSpecificFocus = focusItems.some((item) => normalizedBasePrompt.includes(normalizePromptText(item)))
...
export function mapAfterAnalysisToReviewViewModel(input: {
  ...
  return {
    ...
    confidenceNote: result.confidence_reason || (informationalTask ? (isDeep ? "Deep review is stricter about clarity and completeness." : "Quick review stays directional instead of overconfident.") : isDeep ? "Deep checks what’s actually proven, not just explained" : "Quick review stays directional instead of overconfident."),
    confidenceReasons: result.confidence_reason ? [result.confidence_reason] : [],
    missingItems: buildMissingItems(result, mode),
