import type { AnalyzePromptResponse, ClarificationQuestion } from "./schemas"
...
function extractExclusionTargets(text: string) {
  ...
  for (const pattern of dislikePatterns) {
    for (const match of source.matchAll(pattern)) {
      for (const item of splitIngredientList(match[1] ?? "")) extracted.add(item)
    }
  }
...
function looksLikeOutputFormatHint(value: string) {
  return /step-by-step|steps|ingredients|quantities|html|css|javascript|json|table|bullets|outline|calories|per serving|format|output|list only/.test(value.toLowerCase())
}

function looksLikeStyleHint(value: string) {
  return /clean|polished|professional|readable|realistic|natural|home kitchen|weekday|usable|starter|clear|concise/.test(value.toLowerCase())
}

function isGenericStyleGuardrail(value: string) {
  const normalized = value.toLowerCase().replace(/\s+/g, " ").trim()
  return normalized === "keep the request clear, specific, and easy for the ai assistant to follow." || normalized === "keep the request clear, specific, and easy for the ai assistant to follow"
}

function looksLikeConstraint(value: string) {
  return /no |without |exclude|only|under |less|stovetop|minutes?|servings?|for \d+|limit|keep|must|do not|avoid|[a-z-]+-free/.test(value.toLowerCase())
}
...
function extractPromptSignals(prompt: string) {
  ...
  if (/calories?\s+per\s+serving|list only the total calories per serving/i.test(lower)) outputFormat.push("List only the total calories per serving.")
...
function buildIntentDefaults(intent: AnalyzePromptResponse["intent"]) {
  switch (intent) {
    case "DEBUG":
      return { outputFormat: ["Show the next diagnostic or fix path clearly."], style: ["Keep the scope narrow and testable."] }
    case "BUILD":
      return { outputFormat: ["Return something directly usable as a strong first draft."], style: ["Keep the result clean, practical, and easy to build from."] }
    case "EXPLAIN":
      return { outputFormat: ["Organize the answer so it is easy to follow."], style: ["Use direct, natural wording instead of filler."] }
    default:
      return { outputFormat: [], style: [] }
  }
}

export function buildPromptModeFallbackQuestions(params: { promptText: string; localAnalysis: AnalyzePromptResponse }) {
